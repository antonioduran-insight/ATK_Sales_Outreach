...
根據職缺內容，推斷：
- 他們正在解決什麼問題？
- 這個問題跟 AI Token King 的哪個功能最相關？
- 決策者最在乎的是什麼？

輸出格式：
{
  "pain_point": "推斷的核心痛點（30字內）",
  "product_fit": "AI Token King 對應的解法（30字內）",
  "connection_hook": "LinkedIn 連線請求用語，開頭務必提到職缺，自然、不推銷（50字內）",
  "msg1_body": "連線接受後第一封訊息正文，具體提到職缺描述中的 AI 關鍵字，提供一個具體價值點（100字內）"
}

【任務三：風險評估】
{
  "risk_flag": "無/低/中/高",
  "risk_reason": "如有風險請說明（例：職缺為技術招募而非工具採購、公司規模過小）"
}
...
**D0：連線請求附加訊息（300 字元上限）**
```
{connection_hook}（由 Claude 生成，custom1 欄位帶入）

範本示意：
「看到 {公司} 正在招募 AI 流程自動化相關職位，正好我們在幫台灣中型企業解決 AI 工具導入的效率問題，想交流看看您這邊的方向。」
```

**D3：連線接受後第一封訊息（Message 1）**
```
{msg1_body}（由 Claude 生成，custom2 欄位帶入）

範本示意：
「{名字} 您好，感謝接受連線！

看到貴公司在招募 {職缺名稱} 相關人才，推斷你們正在加速 AI 工具的導入。

我們的產品 AI Token King 幫助企業統一管理 Claude / ChatGPT / Gemini 等多個 LLM 的 API 用量，讓團隊可以在不超出預算的前提下，快速試驗和部署各種 AI 應用。

如果你們正在評估 AI 工具管理方案，5 分鐘的快速交流可能對你們有幫助，方便嗎？」
```

**D7：第一封未回覆後追蹤（Message 2）**
```
「{名字} 您好，上週有分享 AI Token King 的訊息，不知道是否有機會對上您們目前的需求方向？

我們最近剛幫一家規模相近的台灣 SaaS 公司，解決了 AI API 費用失控的問題——他們原本每月 API 費用超出預算 40%，導入後降回正常水位，同時不影響各團隊的使用彈性。

如果有興趣了解細節，或許可以安排 15 分鐘視訊？」
```

**D14：第二封未回覆後追蹤（Message 3）**
```
「{名字} 您好，我知道大家都很忙，最後補充一點：

AI Token King 提供 14 天免費試用，不需要信用卡，也不需要改變現有工具——直接在上面管理貴公司各 AI 工具的 token 用量，看看是否符合你們的需求。

試用連結：aitokenking.com.tw
